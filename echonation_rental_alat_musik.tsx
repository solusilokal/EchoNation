...
const pageData = {
  ...
  catalog: [
    { 
      category: "Gitar & Bass", 
      items: [
        { name: "Fender Stratocaster American Professional", price: "350.000", img: "https://images.unsplash.com/photo-1564186763535-ebb21ef5277f?w=400&h=300&fit=crop" },
        { name: "Bass Gibson Thunderbird", price: "400.000", img: "https://images.unsplash.com/photo-1550291652-6cb9117a4771?w=400&h=300&fit=crop" },
        { name: "Ibanez Premium SR Bass", price: "300.000", img: "https://images.unsplash.com/photo-1516924962500-2b4b3b99ea02?w=400&h=300&fit=crop" }
      ]
    },
    { 
      category: "Drum & Perkusi", 
      items: [
        { name: "Pearl Masterworks Custom Drum Kit", price: "1.200.000", img: "https://images.unsplash.com/photo-1519892300165-cb5542fb47c7?w=400&h=300&fit=crop" },
        { name: "Roland TD-50K2 V-Drums (Elektrik)", price: "950.000", img: "https://images.unsplash.com/photo-1543791187-578b8716b509?w=400&h=300&fit=crop" },
        { name: "Zildjian K Custom Cymbal Set", price: "450.000", img: "https://images.unsplash.com/photo-1524147775535-46f906401026?w=400&h=300&fit=crop" }
      ]
    },
    { 
      category: "Keyboard & Synth", 
      items: [
        { name: "Nord Stage 3 88-Key", price: "850.000", img: "https://images.unsplash.com/photo-1552422535-c45813c61732?w=400&h=300&fit=crop" },
        { name: "Korg Kronos 2", price: "750.000", img: "https://images.unsplash.com/photo-1595069906974-f867bcdd3891?w=400&h=300&fit=crop" },
        { name: "Yamaha Motif XF8", price: "650.000", img: "https://images.unsplash.com/photo-1563330232-57114bb181fa?w=400&h=300&fit=crop" }
      ]
    },
    { 
      category: "Sound System", 
      items: [
        { name: "Paket Akustik (2 Speaker, 1 Mixer, 2 Mic)", price: "1.500.000", img: "https://images.unsplash.com/photo-1520166012956-add9ba0ee3f4?w=400&h=300&fit=crop" },
        { name: "Paket Band Full (FOH, Monitor, Mic Set)", price: "4.500.000", img: "https://images.unsplash.com/photo-1470229722913-7c090be5f524?w=400&h=300&fit=crop" },
        { name: "Paket DJ (2 CDJ, 1 DJ Mixer, 2 Monitor)", price: "2.500.000", img: "https://images.unsplash.com/photo-1598488035139-bdbb2231ce04?w=400&h=300&fit=crop" }
      ]
    }
  ],
  testimonials: [
    { name: "Rizky (Gitaris Indie)", rating: 5, text: "Sewa ampli tabung di EchoNation kualitasnya selalu terjaga. Tabungnya masih fresh, soundnya nendang banget buat manggung." },
    { name: "Sarah (Wedding Organizer)", rating: 5, text: "Sangat membantu! Paket sound system akustiknya praktis, teknisinya ramah dan on-time. Klien saya sangat puas dengan kualitas suaranya." },
    { name: "Deni (Produser)", rating: 4, text: "Katalog keyboardnya lengkap banget. Sewa Nord buat recording album, alat bersih tanpa kendala. Rekomen!" }
  ],
  faqs: [
    ...
    <>
      ...
      <main className="w-full max-w-[480px] mx-auto relative shadow-2xl bg-white min-h-screen overflow-hidden pb-32 border-x border-slate-100">
        ...
        <section className="relative w-full min-h-[90dvh] flex flex-col justify-end pb-12 px-6">
          ...
          <div className="relative z-10 flex flex-col items-center text-center mt-32">
            ...
            <button 
              ...
              className="group relative flex items-center justify-center gap-3 w-full max-w-sm py-4 bg-[#114b79] text-white rounded-xl font-bold text-sm tracking-wide hover:bg-[#2fa0b5] transition-all shadow-[0_0_20px_rgba(47,160,181,0.3)]"
            >
              Sewa Alat Sekarang
          ...
          <div className="flex overflow-x-auto snap-x snap-mandatory gap-3 px-6 mb-8 no-scrollbar">
            {["Semua", ...pageData.catalog.map(c => c.category)].map((cat, idx) => (
              <button
                ...
                className={`snap-center shrink-0 px-5 py-2 rounded-[20px] text-[13px] font-bold transition-all border ${
                  activeCategory === cat 
        ...
        <section id="booking-form" className="py-12 px-6">
          <div className="bg-white border border-cyan-100 rounded-[2rem] p-6 shadow-xl relative overflow-hidden">
            ...
            <form onSubmit={handleFormSubmit} className="flex flex-col gap-4 relative z-10">
              ...
              <button 
                type="submit"
                className="w-full mt-2 bg-[#2fa0b5] text-white font-bold text-sm tracking-wide py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-[#114b79] transition-colors shadow-md border border-[#2fa0b5]/20"
              >
                Kirim via WhatsApp
        ...
        >
          <button 
            onClick={scrollToForm}
            className="w-full flex items-center justify-between px-6 py-3.5 bg-white/95 backdrop-blur-xl border border-[#2fa0b5]/30 rounded-2xl text-[#114b79] shadow-[0_10px_40px_rgba(17,75,121,0.15)] hover:bg-cyan-50 active:scale-[0.98] transition-all"
          >
        ...
        >
          ...
          >
            <div className="flex justify-center items-center mb-6 relative">
              ...
              <button
                onClick={() => setShowShareModal(false)}
                className="absolute right-0 p-1 text-slate-400 hover:bg-slate-100 rounded-full transition-all"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex overflow-x-auto gap-3 pb-2 no-scrollbar items-start px-1 mb-6 justify-center">
              <div className="flex flex-col items-center gap-2 min-w-[76px]">
                <button
                  ...
                  className="w-[56px] h-[56px] rounded-full bg-cyan-50 flex items-center justify-center text-[#114b79] hover:bg-cyan-100 transition-all border border-cyan-100"
                >
              ...
              <div className="flex flex-col items-center gap-2 min-w-[76px]">
                <button
                  ...
                  className="w-[56px] h-[56px] rounded-full bg-[#25D366] flex items-center justify-center text-white hover:brightness-110 transition-all shadow-sm"
                >
